...
Base.getConnectionStringFromEnvironment = function () {
  ...
  return (connectionString) ? connectionString : Base.createConnectionStringFromEnvironment();
};
...
Base.prototype.connect = function (callback) {
  ...
  amqp.connect(me.connectionString, function (error, connection) {
    if (error) {
...
module.exports = {
  ...
  create: function (connectionString, queue) {
    return new Base(connectionString || Base.getConnectionStringFromEnvironment(), queue || Base.getQueueFromEnvironment());
  }
};
